import { NewAccount, User } from '../my-adapter/src/Interfaces';
...
const MyAdapter: Adapter = (options) => {
  const fetchOptions = {
    method: 'POST',
    headers: {
  ...
  return {
    ...
    async createSession({ sessionToken, userId, expires }: { sessionToken:string, userId: string, expires: string }) {
      try {
